...
if (@$mode == 'view') {
    ...
    if (empty($rep_type) && (!empty($where) || empty($where))) {
        ...
        $userid = $_SESSION['customer_userid'];
        $sql = "SELECT v.vehicleid,v.plateno,b.adddate,sum(b.grosspayment)as total,count(b.serviceid) as countservice,s.servicename FROM tbl_billpayment b INNER JOIN tbl_vehicles v ON v.vehicleid=b.vehicleid INNER JOIN tbl_services s ON s.serviceid=b.serviceid WHERE b.deletestatus=1 AND v.adduser='$userid' $where GROUP BY b.vehicleid;";
        $db = dbconn();
        $result = $db->query($sql);
        ?>    
                        ...
                        if ($result->num_rows > 0) {
                            while ($rows = $result->fetch_assoc()) {
                                ...
                                <tr>
                                    ...
                                    <td><a href="customerallvehicleexpenditures.php?mode=view&vehicleid=<?= $rows['vehicleid']; ?>"><?php echo $rows['plateno']; ?></a></td>
    ...
    if (@$rep_type == 'Daily') {
        $userid = $_SESSION['customer_userid'];
        $sqltbl = "SELECT v.plateno, b.serviceid,b.adddate,count(jobcardid) AS count,sum(b.grosspayment) as totalpayment FROM tbl_billpayment b INNER JOIN tbl_vehicles v ON v.vehicleid=b.vehicleid WHERE b.deletestatus='1' AND v.adduser='$userid' $where GROUP BY b.adddate";
    ...
    if (@$rep_type == 'Monthly') {
        $userid = $_SESSION['customer_userid'];
        $sqltbl = "SELECT v.plateno,MONTH(b.adddate) AS month,count(jobcardid) AS count,sum(b.grosspayment) as totalpayment,b.serviceid FROM tbl_billpayment b INNER JOIN tbl_vehicles v ON v.vehicleid=b.vehicleid WHERE b.deletestatus='1' AND v.adduser='$userid' $where GROUP BY MONTH(b.adddate)";
        $result = $db->query($sqltbl);
        ?>
