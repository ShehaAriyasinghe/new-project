...
<?php    
    ...
    $sql = "SELECT v.vehicleid,v.plateno,v.plateimage,v.year,m.vehicletype,m.modelname,b.brandname FROM tbl_jobcards j LEFT JOIN tbl_vehicles v ON j.vehicleid=v.vehicleid INNER JOIN tbl_brands b ON v.brandid=b.brandid INNER JOIN tbl_models m ON v.modelid=m.modelid WHERE v.deletestatus=1 AND j.jobcardstatus='finished' AND j.customeruserid='$userid' $where GROUP BY vehicleid";
    $db = dbconn();
    $result = $db->query($sql);
    ?>    
                    ...
                    if ($result->num_rows > 0) {
                        while ($rows = $result->fetch_assoc()) {
                            ...
                            <tr>
                                ...
                                <td><a href="<?= WEB_PATH ?>vehiclejobcard/view_vehiclejobcards.php?vehicleid=<?php echo $rows['vehicleid']  ?>" class="btn card-btn  btn-sm">View Jobs</a></td>
                                
                                 <td><a href="<?= WEB_PATH ?>vehiclejobcard/view_vehiclerecommendation.php?vehicleid=<?php echo $rows['vehicleid']  ?>" class="btn card-btn  btn-sm">Recommendation</a></td>
