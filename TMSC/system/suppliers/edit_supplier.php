...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && @$operate == "submit") {
        ...
        if (!empty($email)) {
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                $sql = "SELECT * FROM tbl_suppliers WHERE email='$email' && supplierid !='$supplierid'";
                $db = dbConn();
                $result = $db->query($sql);
        ...
        if (!empty($mobile)) {
            if (!preg_match('/((^(\+94)\d{2})[-]\d{7}+$)/', $mobilenum)) {
                ...
                $sql1 = "SELECT * FROM tbl_suppliers WHERE mobile='$mobilenum' && supplierid !='$supplierid'";
                $db = dbConn();
        ...
        if (!empty($cname)) {
            $sql1 = "SELECT * FROM tbl_suppliers WHERE companyname='$cname' && supplierid !='$supplierid'";
            $db = dbConn();
            $result1 = $db->query($sql1);
            if ($result1->num_rows > 0) {
                $messages['error_cname'] = "The Company Name already exist...!";
            }
        }
        ...
        if (!empty($accountno)) {
            ...
            if (!preg_match('/^[-0-9]+$/i', $accountno)) {
                ...
                $sql2 = "SELECT * FROM tbl_suppliers WHERE bankaccountno='$accountno' && supplierid !='$supplierid'";
                $db = dbConn();
        ...
        if (empty($messages)) {
            ...
            $sql = "UPDATE tbl_suppliers SET companyname='$cname',firstname='$fname',lastname='$lname',mobile='$mobilenum',email='$email',address='$address',creditlimit='$creditlimit',bankname='$bname',accountname='$accountname',bankaccountno='$accountno',updatedate='$updatedate',updateuser='$updateuser' WHERE supplierid='$supplierid'";
            $db = dbConn();
            $db->query($sql);

            if (isset($_SESSION['items'])) {
                $sqldel = "DELETE FROM tbl_itemsofsupplier WHERE supplierid='$supplierid'";
                $db->query($sqldel);
                ...
                foreach ($_SESSION['items'] as $key => $value) {
                    ...
                    $sql = "INSERT INTO tbl_itemsofsupplier(supplierid,catalogid,adddate,adduser) VALUES('$supplierid','$key','$adddate','$adduser')";
                    $db = dbConn();
                    $db->query($sql);
    ...
    <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
        <div class="row">
            ...
            <input type="hidden" class="form-control" id="supplierid" name="supplierid" value="<?= @$supplierid; ?>">
            <div class="col-md-4">
                <div class="mb-3">
            ...
            <div class="col-md-6">
                ...
                if (isset($_SESSION['items'])) {
                    foreach ($_SESSION['items'] as $key => $value) {
                        ...
                        <input type="text"  class="form-control-sm w-75" value="<?= $value['itemname'] . "-" . $value['brandname'] . "-" . $value['subcategoryname']; ?>" name="subservice" readonly>
                        <a href="edit_supplier.php?catalogid=<?= @$key; ?>&operate=delete&cname=<?= @$cname ?>&fname=<?= @$fname ?>&lname=<?= @$lname ?>&telephone=<?php echo @$telephone ?>&mobile=<?php echo @$mobile ?>&email=<?= @$email ?>&address=<?= @$address ?>&creditlimit=<?= @$creditlimit ?>&bname=<?= @$bname ?>&accountname=<?= @$accountname ?>&accountno=<?= @$accountno ?>&supplierid=<?= @$supplierid ?>"><i class="bi bi-x-square"></i></a>
