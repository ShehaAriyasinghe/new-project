...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        $sql = "SELECT e.firstname,e.lastname,e.nic,e.mobile,e.image,u.userid,u.username,u.userrole,u.accountstatus FROM tbl_users u INNER JOIN tbl_employees e ON e.userid=u.userid WHERE u.userid='$userid' AND u.deletestatus=1";
        $db = dbConn();
        $result = $db->query($sql);
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($username)) {
            ...
            $messages['error_username'] = "The Username should not be empty...!";
        }
        ...
        if (!empty($username)) {
            if (!filter_var($username, FILTER_VALIDATE_EMAIL)) {
                $messages['error_username'] = "The Username is invalid..!";
            } else {
                $sql = "SELECT * FROM tbl_users WHERE username='$username' AND userid!='$userid'";
                $db = dbConn();
                $result = $db->query($sql);
                if ($result->num_rows > 0) {
                    $messages['error_username'] = "This User name already exists...!";
                }
            }
        }
        ...
        if (empty($messages)) {
            ...
            $sql1 = "UPDATE tbl_users SET username='$username',password='$password',userrole='$jobrole',accountstatus='$accountstatus',updateuser='$updateuser',updatedate='$updatedate' WHERE userid='$userid'";
            $db = dbConn();
            $db->query($sql1);

            $sql2 = "UPDATE tbl_employees SET designation='$jobrole',email='$username',updateuser='$updateuser',updatedate='$updatedate' WHERE userid='$userid'";
            $db->query($sql2);

            $_SESSION['accountusername'] = $username;
            $_SESSION['useraccountrole'] = $jobrole;
            header("Location:user_updsuccess.php");
        }
    }
    ?>
    ...
    <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
        ...
        <div class="row">
            <div class="col-md-8">
                <div class="mb-3">
                    ...
                    <input type="text" name="username" class="form-control form-control-sm" value='<?php echo @$username ?>' id="username">
                    <div class="text-danger"><?php echo @$messages['error_username']; ?></div>
                </div>
                <div class="row">
                    <div class="col-md-4">
                        <div class="mb-3">
        ...
        <div class="row justify-content-around">
            ...
            <div class="col-4">
                <a href="<?= SYSTEM_PATH; ?>useraccounts/user.php" class="btn card-btn">Back</a>
            </div>
        </div>
