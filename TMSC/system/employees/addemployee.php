...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        //advanced validaion
        //Check Email already exists and invalid format
        if (!empty($email)) {
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                $messages['error_email'] = "The Email is invalid..!";
            } else {
                $sql = "SELECT * FROM tbl_employees WHERE email='$email' AND deletestatus='1'";
                $db = dbConn();
                $result = $db->query($sql);
        ...
        if (!empty($nic)) {
            ...
            if ($nic) {
                $sql1 = "SELECT * FROM tbl_employees WHERE nic='$nic' AND deletestatus='1'";
                $db = dbConn();
        ...
        if (!empty($mobile)) {
            if (!preg_match('/((^(\+94|0)\d{2})-?\d{7}+$)/', $mobile)) {
                ...
                $sql1 = "SELECT * FROM tbl_employees WHERE mobile='$mobile' AND deletestatus='1'";
                $db = dbConn();
                $result1 = $db->query($sql1);
                if ($result1->num_rows > 0) {
        ...
        if (empty($messages)) {
            ...
            $sql2 = "INSERT INTO tbl_employees(firstname,lastname,nic,email,address1,address2,city,mobile,designation,image,adduser,joindate) VALUES('$fname','$lname','$nic','$email','$address1','$address2','$city','$mobile','$jobrole','$file_name_new','$adduser','$adddate')";
            $db = dbConn();
            $db->query($sql2);
            header("Location:employee_regsuccess.php");
        }
    }
    ?>
