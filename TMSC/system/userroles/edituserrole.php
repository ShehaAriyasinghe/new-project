...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        if($mode=="edit"){
            $sql = "SELECT * FROM tbl_jobroles WHERE jobroleid='$jobroleid'";
            $db = dbConn();
            $result=$db->query($sql);
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        //Data clean
        $jobrole = cleanInput($jobrolename);
        ...
        //required validation
        if (empty($jobrolename)) {
            $messages['error_jobrole'] = "The Job role name should not be empty...!";
        }
        if (empty($jstatus)) {
        ...
        if (!empty($jobrolename)) {
            $sql = "SELECT * FROM tbl_jobroles WHERE jobrolename='$jobrolename' AND jobroleid !=$jobroleid AND deletestatus=1";
            $db = dbConn();
            $result = $db->query($sql);
        ...
        if (!empty($jobrolename)) {
            if (!preg_match('/^[a-z ]+$/i', $jobrolename)) {
        ...
        if (empty($messages)) {
            ...
            $sql = "UPDATE tbl_jobroles SET jobrolename='$jobrolename',jobavailability='$jstatus',updatedate='$updatedate',updateuser='$updateuser' WHERE jobroleid='$jobroleid'";
            $db = dbConn();
            $db->query($sql);
            showEditSucc();
        }
    ...
    <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
        <div class="mb-3">
            <input type="hidden" class="form-control w-75"  name="jobroleid" value="<?= @$jobroleid; ?>" >
            <label for="jobrole" class="form-label">Enter Job Role</label>
            <input type="text" class="form-control w-75" id="jobrole" name="jobrolename" value="<?= @$jobrolename; ?>" >
            <div class="text-danger"><?php echo @$messages['error_jobrole']; ?></div>
        </div>
        
        <div class="mb-3">
        <label>Select Job Role Availability</label>
