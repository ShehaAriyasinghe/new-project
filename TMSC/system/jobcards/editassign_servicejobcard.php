...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET' && @$mode == 'edit') {
        ...
        $date = date('Y-m-d');
        $sqltbl = "SELECT s.servicename,s.serviceid FROM tbl_jobcards j INNER JOIN tbl_services s ON s.serviceid=j.serviceid WHERE j.jobcardstatus='pending' AND j.deletestatus=1 AND j.reservationdate='$date' AND j.jobcardid='$jobcardid'";
        $db = dbConn();
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($messages)) {
            ...
            if (!empty($_SESSION['servicetasks'])) {
                $sql = "DELETE FROM tbl_jobcardassignserviceslist WHERE jobcardid='$jobcardid'";
                $db = dbConn();
                $db->query($sql);
            ...
            if (!empty($_SESSION['technicianid'])) {
                $technicianid = $_SESSION['technicianid'];

                $sql = "UPDATE tbl_jobcards SET jobcardstatus='pending',technicianid='$technicianid',updateuser='$updateuser',updatedate='$updatedate' WHERE jobcardid='$jobcardid'";
            ...
            if (!empty($_SESSION['subservicetotalprice'])) {
                $subservicestotalprice = $_SESSION['subservicetotalprice'];

                $sql = "UPDATE tbl_jobcards SET jobcardstatus='pending',subservicestotalprice='$subservicestotalprice',updateuser='$updateuser',updatedate='$updatedate' WHERE jobcardid='$jobcardid'";
                //call to the db connection
                $db = dbConn();
                $db->query($sql);
            }
            ...
            header("Location:editassign_successjobcard.php?jobcardid=$jobcardid");
        }
    }
    ...
    <?php
    if ($_SERVER['REQUEST_METHOD'] == 'GET' && @$mode == 'edit' && @$operate != 'servicetaskdelete') {
    ...
    <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
        <div class="row">
            <div class="col-md-6">
                <label class="fs-6">Selected service tasks:</label>

                <?php
                $sql = "SELECT sub.subservicename,sub.subserviceprice,sub.subserviceid FROM tbl_subservices sub INNER JOIN tbl_jobcardassignserviceslist jas ON jas.subserviceid=sub.subserviceid WHERE jas.jobcardid='$jobcardid' AND sub.deletestatus='1'";
                $result = $db->query($sql);
                while ($rows = $result->fetch_assoc()) {
                    ?>
                    <div class="mb-2">
                        <?php echo $rows['subservicename'] . "-" . $rows['subserviceprice']; ?>
                    </div>
                    <?php
                }
                ?>
                            ...
                            <tbody>
                                <tr>
                                    <td>   
                                        ...
                                        <a href="editassign_servicejobcard.php?subserviceid=<?= $key; ?>&operate=servicetaskdelete&mode=<?= $mode ?>&jobcardid=<?= $jobcardid ?>"><i class="bi bi-x-square"></i></a>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
