...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    ...
    <h6>Check all pending job cards</h6>
   ...
   if (@$mode == 'cancel') {
    ...
    if (@$mode == 'view') {
       ...
       $sqltbl = "SELECT j.reservationid,j.jobcardid,j.reservationdate,j.starttime,j.endtime,j.deletestatus,j.serviceid,j.freeservicestatus,j.jobcardstatus,j.vehicleid,c.firstname,c.lastname,r.bay,r.reservationid,s.servicename,s.serviceid,v.vehicleid,v.plateno,v.plateimage,v.year,v.brandid,v.modelid,j.freeservicestatus FROM tbl_jobcards j "
                . "INNER JOIN tbl_services s ON s.serviceid=j.serviceid INNER JOIN tbl_customers c ON c.userid=j.customeruserid "
                . "INNER JOIN tbl_vehicles v ON v.vehicleid=j.vehicleid INNER JOIN tbl_reservations r ON r.reservationid=j.reservationid "
                . "WHERE j.jobcardstatus='pending' AND j.deletestatus=1 AND j.reservationdate='$date' AND j.jobcardid='$jobcardid'";
        ...
        <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
            ...
            <div class="col-8">
                ...
                <a href="pending_jobcards.php" class="btn card-btn">Back</a>
            </div>
        ...
        $date = date('Y-m-d');
        $sql = "SELECT j.reservationid,j.jobcardid,j.reservationid,j.reservationdate,j.starttime,j.endtime,j.deletestatus,j.serviceid,j.jobcardstatus,j.vehicleid,r.bay,r.reservationid,s.servicename,s.serviceid,v.vehicleid,v.plateno,v.plateimage,j.freeservicestatus FROM tbl_jobcards j "
                . "INNER JOIN tbl_services s ON s.serviceid=j.serviceid INNER JOIN tbl_vehicles v ON v.vehicleid=j.vehicleid "
                . "INNER JOIN tbl_reservations r ON r.reservationid=j.reservationid WHERE j.jobcardstatus='pending' AND j.deletestatus=1 AND j.reservationdate='$date' $where";
        $result = $db->query($sql);
        ?>    
                        ...
                        if ($result->num_rows > 0) {
                            while ($rows = $result->fetch_assoc()) {
                                ...
                                <tr>
                                    ...
                                    <td><a href="pending_jobcards.php?mode=view&jobcardid=<?php echo $rows['jobcardid'] ?>" class="btn card-btn btn-sm">View</a></td>
                                    
                                        <td><a href="check_jobcard.php?mode=check&jobcardid=<?php echo $rows['jobcardid'] ?>" class="btn card-btn btn-sm">Check</a></td>
                                  
                                    <td><a href="edit_assignjobcard.php?mode=edit&jobcardid=<?php echo $rows['jobcardid'] ?>" class="btn card-btn  btn-sm">Edit</a></td>
                                    <td><a onclick="return confirm('Are you sure you want to Cancel this jobcard ?');" href="pending_jobcards.php?mode=cancel&jobcardid=<?php echo $rows['jobcardid'] ?>&reservationid=<?php echo $rows['reservationid'] ?>" class="btn card-btn  btn-sm">Cancel</a></td>
                                </tr>
