...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    ...
    if (@$mode == 'cancel') {
        ...
        $sql="UPDATE tbl_servicevoucher SET deletestatus='0' WHERE jobcardid='$jobcardid'";
         $result = $db->query($sql);
    ...
    if (@$mode == 'view') {
        ...
        <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
            ...
            <div class="col-8">
                <?php
                if ($_SESSION['userrole'] == 'supervisor') {
                    ?>
                    <a href="assign_jobcards.php?mode=assign&jobcardid=<?php echo $row['jobcardid'] ?>" class="btn card-btn">Assign</a>
                <?php } ?>
                <a href="view_jobcards.php" class="btn card-btn">Back</a>
            </div>
                        ...
                        if ($result->num_rows > 0) {
                            while ($rows = $result->fetch_assoc()) {
                                ...
                                <tr>
                                    ...
                                    <td><a href="view_jobcards.php?mode=view&jobcardid=<?php echo $rows['jobcardid'] ?>" class="btn card-btn btn-sm">View</a></td>
                                    <?php
                                    if ($_SESSION['userrole'] == 'supervisor') {
                                        ?>
                                        <td><a href="assign_jobcards.php?mode=assign&jobcardid=<?php echo $rows['jobcardid'] ?>" class="btn card-btn btn-sm">Assign</a></td>
                                    <?php } ?>
                                    ...
                                    if ($_SESSION['userrole'] == 'cashier') {
                                        ...
                                        <td><a href="edit_jobcards.php?mode=edit&jobcardid=<?php echo $rows['jobcardid'] ?>" class="btn card-btn  btn-sm">Edit</a></td>
                                    <?php } ?>
                                    <td><a onclick="return confirm('Are you sure you want to Cancel this jobcard ?');" href="view_jobcards.php?mode=cancel&jobcardid=<?php echo $rows['jobcardid'] ?>&reservationid=<?= $rows['reservationid']?>" class="btn card-btn  btn-sm">Cancel</a></td>
                                </tr>
